...
class RenderDocFacade:
    ...
    def save_texture(self, resource_id, output_path, file_type="PNG", mip=-1, slice_index=-1):
        """Save texture data to various formats (PNG, TGA, DDS, JPG, HDR, BMP, EXR)"""
        return self._resource.save_texture(resource_id, output_path, file_type, mip, slice_index)

    def get_mesh_data(self, event_id):
        """Get mesh data for a specific draw call"""
        return self._resource.get_mesh_data(event_id)

    def export_mesh_as_fbx(self, event_id, output_path, attribute_mapping, unit_scale=1, coordinate_system="ue", decode=None, buffer_config=None):
        """Export mesh data as FBX file"""
        return self._resource.export_mesh_as_fbx(event_id, output_path, attribute_mapping, unit_scale=unit_scale, coordinate_system=coordinate_system, decode=decode, buffer_config=buffer_config)

    def export_mesh_csv(self, event_id, output_path, stage="vs_output"):
        """Export mesh data as CSV file"""
        return self._resource.export_mesh_csv(event_id, output_path, stage)
