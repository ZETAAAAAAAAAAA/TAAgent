...
class RequestHandler:
    ...
    def __init__(self, facade):
        ...
        self._methods = {
            ...
            "export_mesh_as_fbx": self._handle_export_mesh_as_fbx,
    ...
    def _handle_save_texture(self, params):
        ...
        mip = params.get("mip", -1)
        slice_index = params.get("slice_index", -1)
        return self.facade.save_texture(resource_id, output_path, file_type, mip, slice_index)
    ...
    def _handle_export_mesh_as_fbx(self, params):
        """Handle export_mesh_as_fbx request"""
        event_id = params.get("event_id")
        output_path = params.get("output_path")
        if event_id is None:
            raise ValueError("event_id is required")
        if output_path is None:
            raise ValueError("output_path is required")
        attribute_mapping = params.get("attribute_mapping")
        if attribute_mapping is None:
            raise ValueError("attribute_mapping is required - AI must analyze CSV and DXBC first")
        unit_scale = params.get("unit_scale", 1)
        coordinate_system = params.get("coordinate_system", "ue")
        decode = params.get("decode")
        buffer_config = params.get("buffer_config")
        flip_winding_order = params.get("flip_winding_order", False)
        return self.facade.export_mesh_as_fbx(int(event_id), output_path, attribute_mapping, unit_scale, coordinate_system, decode, buffer_config, flip_winding_order)
    ...
    def _handle_export_mesh_csv(self, params):
        ...
        if event_id is None:
            raise ValueError("event_id is required")
        output_path = params.get("output_path")
        stage = params.get("stage", "vs_output")
        if output_path is None:
            raise ValueError("output_path is required")
        return self.facade.export_mesh_csv(int(event_id), output_path, stage)
